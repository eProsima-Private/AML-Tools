...
For more information, check the :ref:`AML Dashboard Interfaces <data_collection>` and :ref:`AML Dashboard Usage <amldashboard_usage_data_management>` sections.
...
2. Check the ``activate video`` button in the *webcam* section.

3. Introduce a label for the class that will be recorded in the *Instance label* section.

2. Press the ``Hold to record instances`` button in the *Capture instances to the training set* section.

.. raw:: html

   <video id=myVideo width=100% height=auto autoplay loop controls muted>
      <source src="../../_static/resources/tutorials/dashboard-data-management.mp4">
